...
score1 <- function(data, wgs, params=list(), districts=NULL) {
...
score2 <- function(data, wgs, params=list(), districts=NULL) {
  # data <- read_csv('data/preprocessing/places_wgs84.csv')
  # wgs <- read_csv('data/preprocessing/warsaw_wgs84_every_500m.txt')
  # 
  # params = list(types_in = c('bakery'),
  ...
  for (t in types_in) {
    ...
    tmp$mean_rating[is.na(tmp$mean_rating)] <- 0 #mean(tmp$mean_rating, na.rm=TRUE)
  ...
  for (t in types_out) {
    tmp <- data %>% filter(type==t) %>%
      group_by(wgs) %>% 
      summarise(mean_rating = 5-mean(rating, na.rm=TRUE)) %>%
      arrange(desc(mean_rating))
    tmp$mean_rating[is.na(tmp$mean_rating)] <- 5 #mean(tmp$mean_rating, na.rm=TRUE)
    tmp <- left_join(output, tmp)
...
score3 <- function(data, wgs, params=list(), districts=NULL) {
  # data <- read_csv('data/preprocessing/places_wgs84.csv')
  # wgs <- read_csv('data/preprocessing/warsaw_wgs84_every_500m.txt')
  # 
  # params = list(types_in = c('bakery'),
  ...
  for (t in types_out) {
    ...
    if (nrow(tmp) > 0){
      tmp %<>% summarise(mean_rating = -1) %>%
        arrange(desc(mean_rating))
      tmp <- left_join(output, tmp)
      tmp$mean_rating[is.na(tmp$mean_rating)] <- -1
      tmp$score <- tmp$score + tmp$mean_rating
      output <- tmp %>% select(wgs, score)
    }
  }
...
score3 <- function(data, wgs, params=list(), districts=NULL) {
  ...
  params = list(types_in = c('cafe', 'bakery'),
                types_out = c('police'))
